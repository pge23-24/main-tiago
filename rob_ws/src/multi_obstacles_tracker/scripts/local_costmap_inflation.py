...
class LocalCostMapInflation:
    ...
    def inflateCallback(self, detections: CameraDetectionStampedArray):
        ...
        width_max, height_max = inflated_costmap.info.width, inflated_costmap.info.height
        ...
        for detection in detections.detections:
            ...
            ellipse_center = detection.coordinates[0] * np.array([np.cos(detection.coordinates[1]*np.pi/180.0), np.sin(detection.coordinates[1]*np.pi/180.0)])

            # exclude elements out of the local costmap
            if (np.abs(ellipse_center[0]) > (width_max * resolution)) or (np.abs(ellipse_center[1]) > (height_max * resolution)):
            ...
            for x in range(-width_px//2, width_px//2):
                for y in range(-int(height_px//2), int(height_px//2)):
                    ...
                    T_r_e = np.array([[np.cos(theta), -np.sin(theta), ellipse_center_px[0]], 
                                      [np.sin(theta), np.cos(theta), ellipse_center_px[1]],
                                      [0, 0, 1]])
                    
                    T_m_r = np.array([[np.cos(z_tf), -np.sin(z_tf), width_max/2],
        ...
        reshaped_global_costmap_data_np = np.reshape(global_costmap_data_np, (width_max, height_max)) 

        # apply gaussian filter to the global costmap
        inflation_pixel = int(self.inflation_dist / resolution)
        filtered_reshaped_global_costmap_data_np = gaussian_filter(reshaped_global_costmap_data_np, sigma=2, radius=inflation_pixel)

        # unshape the filtered global costmap
        filtered_global_costmap_data_np = np.reshape(filtered_reshaped_global_costmap_data_np, (width_max * height_max,))

        # get the max between ros local costmap and human detections costmap
        global_costmap_data_np = np.maximum(filtered_global_costmap_data_np, inflated_costmap_data_np)
