    ...
    def compute(self, image):
        if image is not None:
            results = self.model.track(
                image, conf=0.65, persist=True, tracker="bytetrack.yaml"
            )
            return results
...
class MinimalPublisher(Node):
    ...
    def formalize_detection(self, results, height, width):
        ...
        for i in range(n):
            ...
            detections.append(
                ([x1, y1, int(x2 - x1), int(y2 - y1)], score, name)
            )
        return detections
    ...
    def listener_callback(self, image):
        ...
        if results:
            ...
            if self.detector.__class__.__name__ == "YOLOv8Detector":
                annotated_image = results[0].plot()
                new_results = results[0].boxes.data.cpu().tolist()
                class_names = results[0].names
            elif self.detector.__class__.__name__ == "YOLOv5Detector":
                # For information datas
                new_results = results.xyxy[0].cpu().numpy().tolist()
                class_names = results.names
                # For tracker
                res = results.xyxyn[0][:, -1], results.xyxyn[0][:, :-1]
                detections = self.formalize_detection(res, height=cv_image.shape[0], width=cv_image.shape[1])
