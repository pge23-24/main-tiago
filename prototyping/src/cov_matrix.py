...
interp_func = interp1d(
    PIXEL_VALUES, REAL_SIZES, kind="quadratic", fill_value="extrapolate"
)


def pixels_to_distance(pixel_size, object_real_size=1.0):
    """
    ...
    """
    return interp_func(pixel_size) * object_real_size
...
def distance_polar(r1, theta1_deg, r2, theta2_deg):
    """
...
def compute_axis_size(theta_min_deg, theta_max_deg, bb_height, is_major_axis=True):
    """
    ...
    """
    r_min = pixels_to_distance(bb_height, MIN_HUMAN_HEIGHT)
    r_max = pixels_to_distance(bb_height, MAX_HUMAN_HEIGHT)

    if is_major_axis:
        theta_mean_deg = (theta_min_deg + theta_max_deg) / 2
        return distance_polar(r_min, theta_mean_deg, r_max, theta_mean_deg)
    else:
        r_mean = (r_min + r_max) / 2
        return distance_polar(r_mean, theta_min_deg, r_mean, theta_max_deg)
